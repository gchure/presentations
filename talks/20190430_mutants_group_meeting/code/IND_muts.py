...
for m, a in axes.items():
    ax[0, a].set_title(m, fontsize=6, backgroundcolor=colors['grey'], 
                       y=1.08)
ax[0, 0].set_ylabel('fold-change', fontsize=6)
ax[1, 0].set_ylabel('$\Delta F$ [$k_BT$]', fontsize=6)
...
if (ALL_DATA==0) & (KAKI_ONLY==1) & (KAKI_epAI==0):
    plt.savefig('../figs/IND_fit_strain_kaki.pdf', bbox_inches='tight')
if (ALL_DATA==0) & (KAKI_ONLY==0) & (KAKI_epAI==1):
    plt.savefig('../figs/IND_fit_strain_kaki_epAI.pdf', bbox_inches='tight')
if (ALL_DATA==0) & (KAKI_ONLY==1) & (KAKI_epAI==1):
    plt.savefig('../figs/IND_fit_strain_both.pdf', bbox_inches='tight')
if (ALL_DATA==1) & (KAKI_ONLY==0) & (KAKI_epAI==1):
    plt.savefig('../figs/IND_all_data_kaki_epAI.pdf', bbox_inches='tight')
if (ALL_DATA==1) & (KAKI_ONLY==1) & (KAKI_epAI==0):
    plt.savefig('../figs/IND_all_data_kaki.pdf', bbox_inches='tight')
if (ALL_DATA==1) & (KAKI_ONLY==1) & (KAKI_epAI==1):
    plt.savefig('../figs/IND_all_data.pdf', bbox_inches='tight')
if (ALL_DATA==1) &  (KAKI_ONLY==0) & (KAKI_epAI==0):
    plt.savefig('../figs/IND_all_data_only.pdf', bbox_inches='tight')
if (ALL_DATA==0) &  (KAKI_ONLY==0) & (KAKI_epAI==0):
    plt.savefig('../figs/IND_fit_only.pdf', bbox_inches='tight')
