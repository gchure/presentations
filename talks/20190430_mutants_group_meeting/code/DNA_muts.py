...
for m, a in axes.items():
    ax[0, a].set_title(m, backgroundcolor=colors['grey'],
    y=1.08) 
...
for g, d in data.groupby(['mutant', 'repressors']):
    ...
    ax[0, axes[g[0]]].errorbar(d['IPTGuM'], d['mean'], d['sem'], fmt='.',
                               ...
                               ms=8, markeredgewidth=0.5, capsize=1, lw=1, linestyle='none',
...
if ALL_DATA == 1:
    plt.savefig('../figs/DNA_muts_all_data.pdf', bbox_inches='tight')
else:
