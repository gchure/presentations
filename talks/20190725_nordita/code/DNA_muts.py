...
for i in range(3):
    ...
    # Add labels   
    ax[0, i].set_xlabel("IPTG [µM]", style='italic')
    ax[-1, i].set_xlabel("IPTG [µM]", style='italic')

# Add ylabels
ax[0, 0].set_ylabel('fold-change', style='italic')
...
for m, a in corr_ax.items():
    ax[0, a].set_title(m, style='italic', loc='left', y=0.95) 
...
for g, d in data.groupby(['mutant', 'repressors']):
    ...
    ax[0, axes[g[0]]].errorbar(d['IPTGuM'], d['mean'], d['sem'], fmt='.',
                               ...
                               ms=6, markeredgewidth=0.5, capsize=1, lw=0.25, linestyle=':',
...
if ALL_DATA == 1:
    plt.savefig('../figs/DNA_muts_all_data.pdf', bbox_inches='tight', facecolor='white')
# else:
