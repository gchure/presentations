...
for m, a in axes_corr.items():
    ax[0, a].set_title(m, fontsize=8, y=1.03, bbox=title_bbox)
ax[0, 0].set_ylabel('fold-change', fontsize=8)
ax[1, 0].set_ylabel('$\Delta F$ [$k_BT$]', fontsize=8)

for i in range(4):
    ax[0, i].set_xlabel('IPTG [$\mu$M]', fontsize=8)                   
    ax[-1, i].set_xlabel('IPTG [$\mu$M]', fontsize=8)                   
