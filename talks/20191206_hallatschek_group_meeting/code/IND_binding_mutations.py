...
emp_F = pd.read_csv('../data/empirical_F_statistics.csv', comment='#')
emp_F = emp_F[emp_F['class']=='IND']
KaKi_epAI = pd.read_csv('../data/KaKi_epAI_samples.csv', comment='#')
...
ops = ['O1', 'O2', 'O3']
op_edge_colors = {o:colors[f'dark_{c}'] for o, c in zip(ops, base_colors)}
op_fill_colors = {o:colors[f'pale_{c}'] for o, c in zip(ops, base_colors)}
...
if data == True:
    for g, d in fc_data.groupby(['mutant', 'operator']):
        ...
        if g[1] == 'O2':
            ...
            face = op_fill_colors[g[1]]
        if fit == False:
            linestyle = '--'
        else:
            linestyle = 'none'
        _ax.errorbar(d['IPTGuM'], d['mean'], d['sem'], color=op_edge_colors[g[1]],
                    fmt='.', ms=9, linestyle=linestyle, lw=1, markeredgewidth=1,
    ...
    for g, d in emp_F.groupby(['mutant', 'operator', 'IPTGuM']):
        ...
        if (mu > sig) & ((1 - mu) > sig):
           ...
           _ax.vlines(d['IPTGuM'], low, high, color=op_edge_colors[g[1]],
                   lw=1, zorder=zorder) 
           _ax.plot(g[-1], median, marker='.', ms=9, zorder=zorder,
                color=op_edge_colors[g[1]], markerfacecolor=face,
                markeredgewidth=1, label='__nolegend__')
