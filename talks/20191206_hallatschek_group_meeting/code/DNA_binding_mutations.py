...
rep_edge_colors = {r:colors[f'dark_{c}'] for r, c in zip(reps, base_colors)}
rep_fill_colors = {r:colors[f'pale_{c}'] for r, c in zip(reps, base_colors)}

# %%
# Load the fold-change, empirical F, and inference data
fc_data = pd.read_csv('../data/summarized_data.csv', comment='#')
fc_data = fc_data[fc_data['class']=='DNA']
F_data = pd.read_csv('../data/empirical_F_statistics.csv', comment='#')
F_data = F_data[F_data['class']=='DNA']
epRA = pd.read_csv('../data/DNA_binding_energy_summary.csv', comment='#')
...
if data == True:
    for g, d in fc_data.groupby(['mutant', 'repressors']):
        ...
        ax[0, axes[g[0]]].errorbar(d['IPTGuM'], d['mean'], d['sem'], fmt='.',
                                   ms=9, color=rep_edge_colors[g[1]], markerfacecolor=face,
                                   markeredgewidth=1, linestyle=linestyle, label=int(g[1]),
                                   lw=1) 
    ...
    for g, d in F_data.groupby(['mutant', 'repressors', 'IPTGuM']):
        ...
        if (mu > sig) & ((1 - mu) > sig):
            ...
            ax[1, axes[g[0]]].plot(g[-1], median, color=rep_edge_colors[g[1]],
                                  marker='.', ms=9, linestyle=linestyle,
                                  markerfacecolor=face, markeredgewidth=1, alpha=0.85,
                                  zorder=zorder)
