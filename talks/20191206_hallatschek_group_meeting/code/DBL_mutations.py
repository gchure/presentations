...
fc_data = pd.read_csv('../data/summarized_data.csv')
    ...
    for dna, dna_idx in DNA_idx.items():
        for ind, ind_idx in IND_idx.items():
            ...
            ax[dna_idx, ind_idx].errorbar(m_data['IPTGuM'], m_data['mean'], m_data['sem'],
                ...
                ms=9, markerfacecolor=colors['pale_purple'], markeredgewidth=1)
        ...
        for ind, ind_idx in IND_idx.items():
            ...
            for g, d in _data.groupby(['IPTGuM']):
                    ...
                    if g == 0:
                       ...
                       cap_max = 1.5 * g
                       cap_min = 0.6 * g
                    ax[dna_idx, ind_idx + 3].plot(g, _d['median'], '.', 
                                    color=c, alpha=a, ms=9, markeredgewidth=1,
                                    markerfacecolor=colors['pale_purple'])
...
#     plt.savefig('../figs/DBL_predictions.pdf', bbox_inches='tight')
# if (IND_DATA == 1) & (DEL_DATA==0):
#     plt.savefig('../figs/DBL_induction_data.pdf', bbox_inches='tight')
# if (IND_DATA == 1) & (DEL_DATA==1):
#     plt.savefig('../figs/DBL_all_data.pdf', bbox_inches='tight')
